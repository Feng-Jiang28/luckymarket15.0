...
import {
    ProductService as MedusaProductService, Product, Customer,
...
class ProductService extends MedusaProductService {
    ...
    async retrieve(productId: string, config?: FindProductConfig): Promise<Product> {
        ...
        if (product.store?.id && this.loggedInCustomer_?.store_id && product.store.id !== this.loggedInCustomer_.store_id) {
